...
def run_ml():
    st.header('🛠️ Diamonds_ML')
    st.write('''##### ✔️ 인공지능학습시키고 그래프로 실제값과 예측값을 비교하는 페이지입니다.''')
    ...
    if st.button('인공 지능 예측 결과') :
        st.write('''###### ✔️ 0.9806099673065828 로 예측값이 거의 실제값과 근접하다는 것을 의미합니다.''')

    # 비교 그래프 그리기 - Scatter 1
    if st.button('Predict VS Actual_1 Chart') :
        st.write('''###### ✔️ x축에는 학습시킨 예측값을, y축에는 dataset에 있는 price값을 넣어서 표현한 그래프입니다.
            
###### ✔️ 실제와 예측이 거의 비슷한 영역으로 상승하는 그래프임을 볼 수 있습니다.
            ''')
    ...
    if st.button('Predict VS Actual_2 Chart') :
        st.write('''###### ✔️ 위 그래프에서 추가로 cut 영역을 넣어서 해당 영역에 얼마나 밀집되어있는지 확인할 수 있습니다.
            ''')
